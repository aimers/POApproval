sap.ui.define([
	...
	return BaseController.extend("nw.epm.refapps.ext.po.apv.controller.S2_PurchaseOrders", {
		...
		_updatePOSelection: function(oSelectedPO, bIsSelect) {
			...
			if (bIsSelect) {
				...
				for (i = 0; i < aPurchaseOrders.length; i++) {
					if (aPurchaseOrders[i].PoNumber === oSelectedPO.PoNumber) {
		...
		onUpdateFinished: function(oEvent) {
			...
			if (iPOCount === 0) {
				if (!this._sCurrentSearchTerm || this.getModel("appProperties").getProperty("/currentPoNumber") === "") {
					this._showEmptyPage(this._oResourceBundle.getText("ymsg.noPurchaseOrders"));
				}
				return;
			}
		...
		// This method ensures that a PO is selected. This is either the PO with ID sTargetPoNumber or the first PO in the master list.
		_setItem: function() {
			var aItems = this._oList.getItems();
			if (aItems.length === 0) {
				return;
			}
			var i, sTargetPoNumber = this.getModel("appProperties").getProperty("/currentPoNumber"),
				oItemToSelect = aItems[0]; // Fallback: Display the first PO in the list

			if (sTargetPoNumber) { // But if another PO is required: Try to select this one
				for (i = 0; i < aItems.length; i++) {
					if (aItems[i].getBindingContext().getProperty("PoNumber") === sTargetPoNumber) {
			...
			if (this.getModel("appProperties").getProperty("/scrollIntoView")) {
				this._doScrollIntoView(oItemToSelect);
			}
		...
		_doScrollIntoView: function(oItemToSelect) {
			var oDomRef = oItemToSelect.getDomRef();
			if (oDomRef) {
				oItemToSelect.getDomRef().scrollIntoView();
				this.getModel("appProperties").setProperty("/scrollIntoView", false);
			}
		},
		...
		// Parameter sSwipedPoNumber is passed for the second reason to indicate the ID of the removed PO.
		...
		onApprovalExecuted: function(sSwipedPoNumber) {
			if (!Device.system.phone) {
				var oSelectedItem = this._oList.getSelectedItem(),
					nextPoNumber = this._getNextPoNumber(oSelectedItem && oSelectedItem.getBindingContext().getProperty("PoNumber"), sSwipedPoNumber);
				this.getModel("appProperties").setProperty("/currentPoNumber", nextPoNumber);
			}
			this._resetPOSelection();
			...
			// Now (when the app is not being used on a phone) the PO which should be selected after the list update is specified by currentPoNumber.
			// This will be evaluated by method _setItem after the list has been refreshed, which is triggered now.
			this._getListBinding().refresh();
		},

		_getNextPoNumber: function(sSelectedPoNumber, sSwipedPoNumber) {
			var oNextItem, i, aItems = this._oList.getItems(),
				// If sSwipedPoNumber is given the currently selected PO should stay selected if it is not the removed one
				sNextPoNumber = sSwipedPoNumber && sSwipedPoNumber !== sSelectedPoNumber && sSelectedPoNumber;
			for (i = 0; i < aItems.length && !sNextPoNumber; i++) {
				if (aItems[i].getBindingContext().getProperty("PoNumber") === sSelectedPoNumber) {
					oNextItem = aItems[i === aItems.length - 1 ? (i - 1) : (i + 1)];
					sNextPoNumber = oNextItem && oNextItem.getBindingContext().getProperty("PoNumber");
				}
			}
			return sNextPoNumber;
		},
		...
		onSwipeApprove: function() {
			...
			approver.approve(this._oView, true, "", this.onApprovalExecuted.bind(this, oSwipedPO.PoNumber));
		...
		_navToDetails: function(oListItem) {
			...
			oRouter.navTo("PurchaseOrderDetails", {
				POId: encodeURIComponent(oListItem.getBindingContextPath())
			}, !Device.system.phone);
		},
